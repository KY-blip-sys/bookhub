...
// ---------- 下に引っ張ってリロード（スマホ・タブレットのPull to Refresh） ----------
// ヘッダー（.mobile-topbar／PCの.sidebar）はposition:stickyで常に固定されたまま、
// その兄弟要素である#content-and-adだけを指の動きに合わせて下に押し出す。
// 一定距離を超えて指を離すとページを再読み込みする（本物のスクロール可能なアプリの
// 「引っ張って更新」と同じ体感を、ネイティブの挙動が無いPWA環境でも再現するための実装）
function setupPullToRefresh() {
  const contentAndAd = document.getElementById("content-and-ad");
  const indicator = document.getElementById("pull-refresh-indicator");
  if (!contentAndAd || !indicator) {
    return;
  }

  const PULL_THRESHOLD = 64; // これ以上引っ張って指を離すとリロードする
  const MAX_PULL = 110; // 大きく指を動かしても、ここまでしか伸びないようにする（ゴムのような抵抗感）

  let startY = null;
  let pulling = false; // 今まさに「引っ張り」演出の最中かどうか
  let currentDistance = 0;
  let refreshing = false;

  function setDistance(distance) {
    currentDistance = distance;
    contentAndAd.style.transform = distance > 0 ? "translateY(" + distance + "px)" : "";
    indicator.style.opacity = Math.min(distance / PULL_THRESHOLD, 1);
    indicator.style.transform = "rotate(" + distance * 3 + "deg)";
  }

  function reset() {
    contentAndAd.classList.remove("pull-refresh-dragging");
    setDistance(0);
    pulling = false;
    startY = null;
  }
  ...
  document.addEventListener("touchstart", function (event) {
    // ページが一番上までスクロールされていて、サイドバーのドロワーも開いていないときだけ、
    // 「引っ張り」の候補として指の位置を覚えておく（リストのスクロール中などは対象外）
  ...
  document.addEventListener("touchmove", function (event) {
    ...
    if (deltaY <= 0 || window.scrollY > 0) {
      // 上向きに動いた・すでに普通にスクロールしていた場合は、通常のスクロールに任せる
      if (pulling) {
        reset();
      }
      return;
    ...
    pulling = true;
    contentAndAd.classList.add("pull-refresh-dragging");
    // 指を動かした距離をそのまま反映せず、平方根で抵抗をかけて伸びを緩やかにする（自然なゴム感）
    setDistance(Math.min(Math.sqrt(deltaY) * 6, MAX_PULL));
    event.preventDefault(); // ブラウザ標準のバウンス・引っ張りゴムはこちらの演出に置き換える
  }, { passive: false });
